...
import com.involvex.kabelos.audio.AudioService
import com.involvex.kabelos.diagnostics.DiagnosticsLogger
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            "startForegroundService" -> {
                WdCableRuntime.startConnectionService(context.applicationContext)
                result.success("Foreground service started")
            }
            "stopForegroundService" -> {
                WdCableRuntime.stopForegroundService()
                result.success("Foreground service stopped")
            }
            "isForegroundServiceRunning" -> {
                result.success(WdCableRuntime.isForegroundServiceRunning())
            }
            // Kabelos: Diagnostics
            "getDiagnostics" -> {
                val diagnostics = WdCableRuntime.diagnostics()
            ...
            "runSetupCheck" -> {
                ...
                settings["wifiOn"] = wifiManager.isWifiEnabled
                settings["wifiDirectReady"] = context.packageManager.hasSystemFeature(PackageManager.FEATURE_WIFI_DIRECT)
                settings["nearbyPermission"] = permissionManager.hasNearbyWifiDevicesPermission()
                settings["foregroundService"] = WdCableRuntime.isForegroundServiceRunning()
                settings["wifiLock"] = WdCableRuntime.diagnostics()["wifiLockHeld"] == true
                settings["wakeLock"] = WdCableRuntime.diagnostics()["wakeLockHeld"] == true
                settings["samsungNearbyScanning"] = WdCableRuntime.isSamsungNearbyScanningEnabled(context)
                settings["deviceName"] = Settings.Secure.getString(context.contentResolver, "bluetooth_name") ?: Build.MODEL
                settings["deviceModel"] = Build.MODEL
                settings["androidVersion"] = Build.VERSION.RELEASE
                result.success(settings)
            }
            // Kabelos: QR pairing payload
            "getQrPayload" -> {
                val deviceName = Settings.Secure.getString(context.contentResolver, "bluetooth_name") ?: Build.MODEL
                val pairingCode = (10000000..99999999).random().toString()
            ...
            "openNearbyDevicesSettings" -> {
                ...
                result.success("Opened nearby devices settings")
            }
            "openAppSettings" -> {
