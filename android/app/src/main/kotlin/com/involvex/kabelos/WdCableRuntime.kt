...
object WdCableRuntime {
    ...
    fun startForegroundService(activity: Activity) {
        startConnectionService(activity)
    }

    fun stopForegroundService() {
        val ctx = synchronized(lock) { appContext } ?: return
        stopConnectionService(ctx)
    }

    fun isForegroundServiceRunning(): Boolean {
        return WdCableConnectionService.isRunning
    }

    fun isSamsungNearbyScanningEnabled(context: Context): Boolean {
        return try {
            val pm = context.packageManager
            val settings = context.packageManager.getApplicationInfo("com.samsung.android.settings", 0)
            true
        } catch (e: Exception) {
            false
        }
